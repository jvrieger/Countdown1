...
const bookList = [
  ...
  */
  ...
  const add = (entry) => bookList.push(entry);
  add({title: "myTitle", author: "myAuthor"});
  
  // (2) Write an arrow function that removes a specific book from the bookList
  const remove = (indexToRemove) => bookList.splice(indexToRemove, 1);
  remove(2);

  // (3) Write an arrow function that lists out all the books or all the authors in the book list */
  const list = bookList.map(obj => obj.title);
  console.log(list);
  ...
  /* (4) Write a one-line arrow function that takes in a number and returns a string stating whether the
    ...
    const pos = (number) => number % 2 === 0 ? `${number} is even!` : `${number} is odd!`;
    console.log(pos(3));
    console.log(pos(6));
  ...
  /* (5) Write a switch statement for a 'day' variable that prints out something based off of what day of
    ...
    switch (day) {
      ...
      case 'Friday': 
        console.log("friyay!!");
      default:
        console.log("that day hasn't been added!");
    }
